...
@torch.no_grad()
def test_dataloader_process(denoiser, dataloader, file_manager, cfg, add_con=0., floor=False, img_save=True,
                            img_save_path=None, info=True, logger=None, status=None):
    '''
...
def set_optimizer(module, train_cfg):
    optimizer = {}
    for key in module:
        optimizer[key] = optim.Adam(module[key].parameters(), lr=float(train_cfg['init_lr']),
                                    betas=train_cfg['optimizer']['Adam']['betas'])
    return optimizer
...
def warmup(model, loss, train_dataloader, optimizer, logger, max_iter, epoch, max_epoch, loss_dict, loss_log, tmp_info,
           tboard, cfg):
