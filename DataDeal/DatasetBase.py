...
class RotateTransformDataset(Dataset):
    def __init__(self, dataset, rotate_prob=0.25):
        self.dataset = dataset
        self.rotate_prob = rotate_prob

    def __getitem__(self, idx):
        sample = self.dataset[idx]
        # 如果样本是元组或列表，假设第一个元素为图像数据，其余为标签等信息
        if isinstance(sample, (tuple, list)):
            data, *rest = sample
            if random.random() < self.rotate_prob:
                # 使用 torch.rot90 将图像旋转90度
...
def set_dataloader(dataset_class, dataset_cfg, batch_size, shuffle, num_workers):
    ...
    for key in dataset_dict:
        ...
        # 包装数据集，使其以25%的概率旋转90度
        dataset = RotateTransformDataset(dataset, rotate_prob=0.25)
        dataloader[key] = DataLoader(dataset=dataset,
