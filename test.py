...
def test_img(denoiser, image_path, save_dir='./'):
    ...
    cv2.imwrite(os.path.join(save_dir, name + '.png'), denoised)
    # print message
...
def test_DND(img_save_path, file_manager):
    ...
    DND_infor_path = cfg['data_root_dir'] + '/DND'
...
def test():
    ...
    # -- [ Test Normal Dataset ] -- #
        ...
        if psnr is not None and ssim is not None:
            with open(os.path.join(file_manager.get_dir(img_save_path), '_psnr-%.2f_ssim-%.3f.result' % (psnr, ssim)),
                      'w') as f:
                f.write('PSNR: %f\nSSIM: %f' % (psnr, ssim))
...
if __name__ == '__main__':
    ...
    args.add_argument('-sf','--save_folder', default='output/test_ambsn', type=str)
    args.add_argument('-p','--pretrained', default='ckpt/AMBSN.pth', type=str)
    args.add_argument('--thread', default=8, type=int)
    args.add_argument('--self_en', action='store_true')
    args.add_argument('-td', '--test_dir', default='/data/wyj/SIDD_val/real_noisy', type=str)
    args.add_argument('-rd', '--data_root_dir',
                      default='/data/wyj/dataset/SIDD/val', type=str)
    ...
    test_cfg = cfg['test']
    # test_cfg['dataset'] = 'DND_benchmark'
    ...
    test()
    #
    # test_DND(img_save_path='.output/DND_test', file_manager = FileManager(output_folder))
